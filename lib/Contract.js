 ...
 */
module.exports.account = function ( callback ) {
    function return_account( dto ) {
        callback( dto.accountId.replace(/^act_/,'') );
    }
    var path = papi + "contracts";
    EdgeGrid.get( path, return_account ).end();
}
 ...
 */
module.exports.cpcodes = function ( contractId, groupId, callback ) {
    var path = '/papi/v1/cpcodes?contractId=' + contractId + "&groupId=" + groupId;
    EdgeGrid.get( path, callback ).end();
}
